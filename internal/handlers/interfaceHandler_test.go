...
func TestMutex(t *testing.T) {
	...
	var dummyKeys = []string{"a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"}
	...
	for sendOrder, eachDummyKey := range dummyKeys {
		...
		requestBodyInString := fmt.Sprintf(requestDataFormat, "a"+eachDummyKey, "dummy"+strconv.Itoa(sendOrder))
	...
	*/
	for sendOrder, eachDummyKey := range dummyKeys {
		...
		reqeustToInterface, err := http.NewRequest(http.MethodGet, "http://localhost:8001/hash/data/a"+eachDummyKey, &requestBody)
		if err != nil {
		...
		fmt.Printf("GET a%s 요청 결과 : %s 노드에서 %s 받아옴\n", eachDummyKey, responseContainer.HandleNode, responseContainer.Response)
		// fmt.Printf("GET a%s 요청 결과 : %s \n", eachDummyKey, string(responseDataByte))
